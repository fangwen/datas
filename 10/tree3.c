...
void print(Tree *tree)
{
	int i;
    int j = 0;
    char *number[] = {"file1", "file2", "file3", "file4", "file5", "file6", "file7", "file8"};
    for (i = 0; i < tree->n; i++) {
...
int main()
{
    ...
    for (i = 0; i < (&tree)->n; i++) {
        int j;
        for (j = i; j < (&tree)->n; j++) {
            if (((&tree)->node[i].x - (&tree)->node[j].x) * ((&tree)->node[i].x - (&tree)->node[j].x)
                + ((&tree)->node[i].y - (&tree)->node[j].y) * ((&tree)->node[i].y - (&tree)->node[j].y) <= 2) {
                int p = find(&tree, i);
                int q = find(&tree, j);
                if(p != q)
                    merge(&tree, q, p);
