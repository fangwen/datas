...
void creat_matrix(Crosslist *mat)
{
    ...
    for (k = 0; k < mat->terms; k++) {
        ...
        if(mat->rhead[i] == NULL || mat->rhead[i]->j > j) {
            ...
            struct olnode *q = (struct olnode *)malloc(sizeof(struct olnode));
            for (q = mat->rhead[i]; (q->right != NULL) && (q->right->j < j); q = q->right) {
                p->right = q->right;
                q->right = p;
            }
        }
        ...
        /* } else { */
        /*     struct olnode *q = (struct olnode *)malloc(sizeof(struct olnode)); */
        /*     for (q = mat->chead[j]; (q->down != NULL) && (q->down->i < i); q = q->down) { */
        /*         p->down = q->down; */
        /*         q->down = p; */
        /*     } */
        //        }
